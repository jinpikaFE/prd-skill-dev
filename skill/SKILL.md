...
- Keep the generated template's `.prd-template.json` and locked workbench files unchanged. Ordinary feature generation may modify only `src/prototype/`, `src/data/prdData.ts`, `src/data/generatedDocs.ts`, generated documents, review JSON, and version directories. Run the package validator to detect workbench drift.
