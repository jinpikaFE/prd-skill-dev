...
- `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `vite.config.ts`, `tsconfig.json`, `index.html`, and `src/`: Vue3 + Vite + TypeScript + Pinia + Vant + markdown-it + Mermaid PRD workspace source copied from `assets/vue3-prd-template/` and adapted to the feature.
...
- Use Vant components for review-workbench form controls whenever Vant has a fitting component, including history, role, scenario, state, and requirement selectors. Do not use native `select` for these controls in generated Vue3 PRD workspaces.
...
4. Build or adapt the high-fidelity Vant prototype first so the primary product interaction is complete and demoable.
...
6. Copy and adapt `assets/vue3-prd-template/` into the output folder. Fill `src/data/prdData.ts`, `review-data/draft.json`, `versions/index.json`, and the documents from the same requirement facts. Include left feature navigation, top history/view switching, a top release button with second confirmation, file-backed draft and per-version snapshots, finalized-version readonly mode, duplicate version-name prevention, deletable/renameable version records with second confirmation, Vant-based selectors, zoomable/draggable annotated PRD canvas with drag mode selected by default, product-added annotations, high-fidelity prototype view, in-page document viewing and download, annotation/comment creation, annotation/comment editing and deletion, right all-annotation and all-comment lists, click-to-canvas locating, current requirement IDs, traceability, and version notes.
