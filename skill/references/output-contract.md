...
- No Vant imports or Vant CSS in `src/main.ts`, `src/App.vue`, or `src/workbench/`. The locked `src/prototype/main.ts` bootstrap loads Vant CSS only for mobile prototypes and registers Ant Design Vue plus its reset only for desktop prototypes. This iframe boundary prevents product resets and component styles from changing the workbench `body`, menus, fields, buttons, and dialogs.
- Preserve `.prd-template.json` and every listed `lockedFiles` hash, including `src/prototype/main.ts`. Generated feature work belongs only in the manifest's extension paths, excluding locked files within those paths. The validator must fail when a locked template file drifts or when a prototype relies on parent scoped styles for `defineComponent + h()` children.
