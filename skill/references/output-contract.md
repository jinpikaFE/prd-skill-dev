...
The root is the Vue3 prototype project. Do not generate a separate single-file HTML PRD mode. `index.html` is the Vite app entry, not the full prototype implementation.
...
The default stack is Vue3 + Vite + TypeScript + Pinia + Vant + markdown-it + Mermaid. Vant is the default mobile UI component source for high-fidelity prototype surfaces; Mermaid is the default third-party renderer for PRD flowcharts and, when the requirement structure benefits from it, mindmaps. Do not treat either dependency as a requirement for the user's real production repository unless the user asks for that.
...
- Top history selector for finalized versions plus a release button near the selector. If no version has been finalized, show the current draft and make it clear that history appears only after the user says “定版” or uses the page release action. Use Vant picker/popup-style controls for history and review selectors rather than native `select` when Vant has a matching component.
...
- High-fidelity prototype view: the interactive product UI for customer demonstration, built first and using Vant mobile components where they fit.
...
- `meta`: feature name, slug, version, updated time, draft/final state.
