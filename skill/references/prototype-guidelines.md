...
- Include a `FolderOpenOutlined` icon action beside the history controls. Its Ant Design Vue `Tooltip` should say “打开当前原型所在文件夹”; the local Vite PRD file API opens the workspace root for draft or the selected version directory for a finalized snapshot. The browser may send only a version ID, never an arbitrary filesystem path.
- Page release freezes the current draft into an independent version directory. The snapshot must at least include product-added annotations, generated annotation edits, deleted annotation IDs, and comments. Viewing a finalized version must read `versions/vX.Y.Z/review-data.json`, not the current draft JSON. Product annotations are read-only, while the canvas remains draggable and zoomable and anonymous review comments may still be added. Post-release comments are append-only, store `createdAt`, and live in `versions/vX.Y.Z/review-comments.json`.
...
- Use Ant Design Vue `Tooltip` for workbench icon actions and other explanatory hover hints. Do not use native `title` attributes on buttons, menu items, links, or form controls. Keep visible `Modal` titles, iframe accessibility names, Vant tab labels, and business data `title` fields as their native component/data APIs.
- Reuse `src/workbench/components/internal/EllipsisTooltipText.vue` for intentional text truncation. It defaults to one line, accepts an explicit `rows` value for multi-line clamping, observes size changes, and displays the full-text Tooltip only when `scrollWidth/clientWidth` or `scrollHeight/clientHeight` confirms overflow. Use `tooltip-trigger="always"` only when the hint conveys extra explanatory content rather than merely revealing truncated text.
- Ant Design Vue `Select` adds a browser-native `title` when its selected label or option label is a plain string. For long labels such as version history, render the option label as an `EllipsisTooltipText` component node so both the selected value and dropdown option use Ant Design Vue Tooltip instead.
- Use the locked `RequirementTag` component for workbench `REQ-###` tags. The Ant Design Vue Tooltip should resolve the shared requirement data and show the requirement title, description, and first acceptance criterion; do not duplicate requirement copy in the view component.
- Support adding temporary comments on PRD annotations. Store draft comments in Pinia state and persist the draft to `review-data/draft.json` through the local Vite PRD file API. Every comment must store and visibly display its creation date.
...
- The local header exposes a platform-neutral “生成发布包” action. It packages finalized history into `publish/<project>-review-<timestamp>.zip`, shows the absolute package path, and provides a copyable Codex deployment prompt. It must not request provider accounts or deploy directly.
- The review ZIP contains `published-state.json` and `deployment-handoff.json`. Hosted mode has no draft, hides local file/version/package actions, and lists finalized history only. Anonymous comments remain disabled until the chosen deployment platform implements the generic state/comment endpoints in the handoff file.
